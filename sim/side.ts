...
export class Side {
	...
	constructor(name: string, battle: Battle, sideNum: number, team: PokemonSet[]) {
		...
		this.dynamaxUsed = this.battle.gen <= 7;
	...
	canDynamaxNow(): boolean {
		if (this.battle.gen <= 7) return false;
		...
		if (dynamax && (this.choice.dynamax || !canDynamax)) {
			if (pokemon.volatiles['dynamax']) {
				...
				if (this.battle.gen <= 7) {
					return this.emitChoiceError(`Can't move: Dynamaxing doesn't outside of Gen 8 or later.`);
